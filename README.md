...
`db:migrate` applies Prisma migrations to local SQLite (`packages/server/prisma/dev.db`). F1 → … → F7 team setup → **F8 lineups/secondary positions**. Do not commit `*.db` files.
