...
**F26 (2026-07-15):** `/scouting` (pick a club) and `/teams/:teamId/scouting` host **Team-Private Scouting Estimates** — Commissioner-managed Scouts/departments, versioned immutable calibration, PLAYER/COUNTRY/WATCHLIST assignments, deterministic observations, confidence-bounded estimate ranges, append-only versioned reports, staleness after Player changes + rescouting, team-private watchlists and manual rankings, estimate-only suggested rankings, and Commissioner true-comparison diagnostics. Normal APIs never expose true prospect values.

**F27 (2026-07-15):** `/drafts` (current-season status + latest selections) and `/drafts/:draftEventId` (Overview/Eligible Prospects/Draft Order/Lottery/Draft Room/Results/Team Board/Diagnostics) host the **Annual Amateur Draft** — versioned draft configuration, explicit-cutoff eligibility snapshots, reverse-standings/MANUAL order, bounded deterministic seeded lottery, multi-round picks (original==current owner — no pick trading), ACTIVE `PlayerDraftRight` (no contract, no club assignment), frozen team board snapshots at draft start (estimates only), estimate-only deterministic auto-pick, Commissioner lifecycle (create → eligibility → order → lottery → ready → start → picks → complete), pre-start SQLite backup, and the team-scoped Draft Room (manual Select + Auto-Pick + Commissioner Select). Drafted players remain `PROSPECT`, unsigned, and `currentTeamId = null`. No contracts, trades, pick transfers, club assignment, lineup mutation, or next-WorldSeason creation.
...
The live client adds `/contracts`, `/contracts/:contractId`, `/teams/:teamId/contracts`, and `/free-agency`, with Overview/Active/Expiring/Transactions/Expiration/Configuration/Diagnostics contract tabs. It supports compatibility initialization, explicit-season expiration, team offer creation/submission/acceptance/withdrawal, team contract/payroll views, player-history links, versioned configuration display, and Commissioner boundaries. Payroll is informational and every surface states that no salary cap is enforced. Rights signing is exposed by the contract API and remains distinct from draft history.

## F29 implementation note (2026-07-16)

The live client adds `/trades` (Overview/Proposals/Completed tabs), `/trades/:tradeId` (immutable completed-trade detail with asset transfers + transaction history), `/trade-proposals/:proposalId` (both-side assets, Team-context advisory valuation with fairness warning, and submit/withdraw/accept/reject actions gated by the acting club), and `/teams/:teamId/trade-center` (team overview + a player-for-player New Proposal builder). It supports two-club proposals with immutable asset snapshots, deterministic Team-specific valuations using each club's F26 scouting estimates or a conservative Unknown fallback, atomic acceptance with a pre-trade backup, ACTIVE+FUTURE contract transfer synchronized with `Player.currentTeamId`, `DraftPick.currentTeamId` transfer with `originalTeamId` preserved, and ACTIVE draft-right transfer without signing the Player. Every surface states that trade value is advisory only and that there is no salary cap, retained salary, conditional picks, multi-team trades, autonomous AI, or lineup auto-rewrite. Commissioner routes manage configuration versions, accept on a Team's behalf, and reveal both-side diagnostics behind the gate; scouting reports are Team-private and never transfer.

## F30 implementation note (2026-07-17)

The live client adds `/offseason` (current WorldSeason, run status, progress %, Commissioner create/start/refresh/complete controls), `/offseason/runs/:runId` (Checklist tab with per-phase required/category/status/linked-operation/blockers columns plus Start/Complete/Skip actions gated by Commissioner Mode and readiness; History tab of append-only orchestration events; Teams tab linking to per-team pages; Final Review tab aggregating blockers/warnings with the explicit "completing F30 does not create the next WorldSeason — F31 will" notice), and `/offseason/runs/:runId/teams/:teamId` (privacy-safe per-team summary of contracts, free agency offers, draft rights, trades, retired players, roster blockers, lineup blockers, and stale scouting). Every surface states that F30 coordinates existing subsystems without duplicating their logic, that required phases cannot be skipped, that completed phases/runs are immutable, and that there is no salary cap, roster-size cap, auto-accept, auto-trade, auto-draft-pick, auto-release, auto-lineup, or auto-rescout. Commissioner routes own run create/start/cancel/refresh/complete, phase start/complete/skip/retry/link, configuration CRUD + version activate, and run diagnostics; normal mode is read-only. The sidebar gains an Offseason entry.

## F31 implementation note (2026-07-17)

The live client adds `/seasons` (current-season card with F30-completed prerequisite + transition-eligibility readiness; full season timeline with order/label/years/status/current marker; latest-transition card), `/seasons/:worldSeasonId` (season overview, readiness, and the transition runs that produced or consumed it — planned structures only), `/season-transition` (Commissioner preview → prepare → execute launcher with the explicit banner "Creates the next WorldSeason — does not generate schedules or simulate competitions — a safety backup will be created — completed transition cannot be undone in the UI"; preview shows deterministic target identity, CompetitionEdition carry-forward list, stage templates, participant counts, contract/right summary, free agents, scouting staleness, national-team warnings, blockers/warnings, and input/plan hash prefixes), and `/season-transition/runs/:runId` (source/target, status, config + hash prefixes, backup metadata, entity summary, and append-only event history). Every surface states that exactly one transition per source season is allowed, that completed transitions are immutable, that no schedules/Matches/standings/brackets are generated, that F24–F30 operations are not replayed, that FUTURE contracts are not auto-activated, that lineups are not auto-rebuilt, and that locked national-team rosters are not reused. Commissioner routes own prepare/execute/cancel/retry, configuration CRUD + version activate, and run diagnostics; normal mode is read-only. The sidebar gains a Seasons entry.

## F33 implementation note (2026-07-18)

The live client adds `/maintenance` (Commissioner-managed Data & Maintenance surface with Overview/Export/Import/Database Validation/Initialization Reset/History/Configuration tabs). Overview shows bounded subsystem health (configured, completed exports, pending imports, last full-DB package age, last validation status/age) — no filenames, paths, hashes, or fingerprints. Export walks type → filters → preview (estimated rows, columns, privacy warning) → generate → download; clearly labels Public-safe vs Commissioner truth vs Full database package, with a warning banner on truth exports. Import shows the upload → preview (valid/warning/error counts + sample issues + duplicate policy) → mandatory F32 backup notice → apply (disabled while blockers remain) flow. Database Validation runs read-only checks grouped by domain (SQLite/migrations, world, players/teams, contracts, draft, trades, competitions, statistics, scouting, offseason/transitions, backups, maintenance) with PASS/WARNING/FAIL status and a JSON diagnostic download. Initialization Reset is a clearly separated destructive area: preview shows exact affected counts + fingerprint prefix + required typed confirmation phrase `RESET WORLD <short id>` + the explicit "F32 restore is the only recovery path" notice. Every surface states that exports never mutate world data, that public-safe exports omit hidden truth, that imports apply atomically after a VERIFIED F32 backup, that database validation never silently repairs, that full-DB export does not bypass F32 restore, and that reset preserves migrations + backups + configuration. The sidebar gains a "Data & Maintenance" entry.

Server-side, F33 adds 9 services + 2 route files (public `/api/system/maintenance-status` + Commissioner `/api/commissioner/maintenance/*`) covering 16 export types (PLAYERS_PUBLIC/COMMISSIONER JSON+CSV, TEAMS/STANDINGS/PLAYER_STATISTICS/GOALIE_STATISTICS/CONTRACT_HISTORY/DRAFT_HISTORY/TRADE_HISTORY/TRANSACTION_HISTORY CSV, COMPETITION_ARCHIVE/CONFIGURATION_PRESET/NAME_POOLS JSON, FULL_DATABASE_PACKAGE ZIP), validated name-pool + configuration-preset imports (preview → atomic apply with mandatory F32 backup), read-only database validation, full-DB `.zip` packaging through F32, and initialization reset (RESET_SETUP_STATE_ONLY / RESET_WORLD_TO_EMPTY). All writes require Commissioner Mode + reason + safe paths + no arbitrary SQL. `@fastify/multipart` handles uploads; `archiver` produces real `.zip` packages. Public `/api/system/maintenance-status` exposes only bounded metadata.

## F32 implementation note (2026-07-17)

The live client adds `/backup-recovery` (Commissioner-managed Backup & Recovery surface with Overview/Backups/Create Backup/Restore/Retention/Storage Scan tabs). Overview shows bounded subsystem health (configured, verified count, corrupt/missing count, last-verified age, pending restore, maintenance mode) — no filenames, paths, hashes, or fingerprints. Backups is a filterable table (created, type, reason, status, size, WorldSeason, hash prefix, protected, verify/protect/unprotect actions). Create Backup creates a SQLite-safe `VACUUM INTO` snapshot with an explanation that backup creation never mutates world data and is verified before availability. Restore walks preview (compatibility, fingerprints, data-loss warning, restart requirement) → typed confirmation phrase `RESTORE <short id>` → prepare → request-restart (maintenance banner + restart instructions). Retention shows a deterministic prune preview (keep/prune/protected counts) with explicit-reason execution. Storage Scan lists orphan/missing/corrupt findings with no automatic deletion. Normal-mode users see only the bounded system status; Commissioner Mode is required for every mutation. The sidebar gains a Backup & Recovery entry.

Server-side, F32 centralizes every prior ad-hoc pre-operation safety snapshot into one `createDatabaseBackup` service (VACUUM INTO → file SHA-256 → canonical manifest + manifest SHA-256 → `PRAGMA integrity_check` → migration-table check → deterministic database fingerprint → VERIFIED) that all 12 world-mutating operations (F18/F19/F20/F21/F23/F24/F25/F27/F28×2/F29/F31) route through with source-operation type+id and idempotent VERIFIED-linked-backup reuse on retry. Restore is restart-required: prepare (re-verify + mandatory protected PRE_RESTORE backup + external recovery journal + restore marker) → request-restart (maintenance mode) → pre-Prisma startup bootstrap (atomic file replacement + emergency-copy rollback + additive migration + fingerprint verification + history reconciliation + marker cleared only after success). Public `/health` and `/api/system/backup-status` expose only bounded metadata (no filenames/paths/hashes/fingerprints/operation details).
