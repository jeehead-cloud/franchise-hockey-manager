import { getChemistryWeights } from './config.js';
import type { ChemistryPlayerInput } from './types.js';

export function computeBaseAbility(players: ChemistryPlayerInput[]): number {
  if (players.length === 0) return 0;
  const avgAbility =
    players.reduce((s, p) => s + p.currentAbility, 0) / players.length;
  const avgRole =
    players.reduce((s, p) => s + p.roleRating, 0) / players.length;
  const contribution = getChemistryWeights().roleRatingBaseContribution;
  // Small centered adjustment from role rating (0–100 → roughly ±contribution*50)
  const adjusted = avgAbility + (avgRole - 50) * contribution;
  return Math.round(Math.min(100, Math.max(0, adjusted)) * 10) / 10;
}

export function computeEffectivePerformance(opts: {
  baseAbility: number;
  ...
  const caps = getChemistryWeights().caps;
  ...
  const raw =
    chemistryContribution + coachFitContribution + tacticalFitContribution;
