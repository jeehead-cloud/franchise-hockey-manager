...
describe('Migrations', () => {
  it('applies all migrations to a fully empty SQLite database', async () => {
    ...
    try {
      ...
      expect(names).toContain('OffseasonPreset');
      expect(names).toContain('OffseasonPresetVersion');
      expect(names).toContain('ActiveOffseasonConfiguration');
  ...
  it('records F1–F28 migrations in history', async () => {
    ...
    expect(names.some((n) => n.includes('f33_import_export_database_maintenance'))).toBe(true);
