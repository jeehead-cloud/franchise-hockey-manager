...
**F33 import/export maintenance (local):**

- `FHM_EXPORT_DIR` — export directory for the F33 maintenance subsystem (default `.fhm-exports/` at repo root; gitignored). Holds all generated export files (`.csv`/`.json`/`.zip`), their canonical-JSON sidecar manifests, the `.staging/` subdir for multipart upload isolation, and the external `maintenance-reset-journal.json` (survives RESET_WORLD_TO_EMPTY because deleting the world deletes the reset-run row that requested it).
- **F33 is SQLite-only and local-only.** No cloud/off-site export durability, no real-world roster scraping, no 3rd-party sports APIs, no full-DB-package import into a live DB, no record-level restore, no arbitrary SQL console, no save slots. Exports are portability/diagnostic artifacts — restore remains an F32 workflow.
- **Filesystem permissions:** the server process needs read+write access to `FHM_EXPORT_DIR`. The directory should be on the same volume as the active database so writes are atomic.
- **Required free disk space:** at minimum, enough for the configured retention ceiling (`retention.maximumGeneratedExports`, default 100 files × up to `limits.maximumExportRows`, default 1M rows) plus staging space for the largest expected upload (`limits.maximumImportBytes`, default 100 MiB) plus one full-database `.zip` package. A safe floor is `5 × active_db_size`.
- **Upload directory:** `<FHM_EXPORT_DIR>/.staging/` holds multipart upload files between the upload step and the apply step. Staging files are removed after successful apply or cancellation; orphaned staging files from interrupted runs remain until manually cleared.
- **Generated export retention:** `pruneOldExports(config)` deletes COMPLETED exports older than `retention.maximumAgeDays` (default 30) beyond the `retention.maximumGeneratedExports` (default 100) newest. Pruning never deletes outside the configured export root and never deletes the active database.
- **Temporary upload directory:** see `.staging/` above — bounded by `limits.maximumImportBytes`.
- **Git exclusions:** `.fhm-exports/`, `*.db`, `*.db-journal`, and `.fhm-backups/` are gitignored. Never commit export files, manifests, uploads, staging files, full-DB packages, or the reset journal.
- **No cloud/off-site export durability:** F33 does not push exports anywhere off the local machine. The owner is responsible for copying `.fhm-exports/` off-host if they want off-site durability.
- **Full database package relationship to F32:** the `.zip` package invokes the centralized F32 `createDatabaseBackup` for a VERIFIED SQLite snapshot and packages it with manifests/checksums. Restoring a package into a live DB is **not** performed by F33 — extract, verify SHA-256 against `package.manifest.json`, then use F32 Backup & Recovery to restore from the extracted database file. F33 does not bypass F32 restore semantics.
- **Reset safety notes:** RESET_WORLD_TO_EMPTY requires Commissioner Mode + the typed phrase `RESET WORLD <short id>` (derived from the current WorldSeason start year) + a current-database-fingerprint confirmation + no running world operation + no pending restore. A mandatory protected F32 backup is created before any deletion. Reset deletes domain rows in foreign-key-safe order in one transaction (not a DB file replacement — no restart required). Reset never deletes: the backup directory, the export directory, recovery journals, migration files, source dataset files, F32 backup files, or any `*Preset*`/`Active*Configuration` table. AppMeta init flags are cleared so the next setup is a clean first init.
